...
@section('content')
    <div class="page">
        ...
        <aside class="recognition" aria-label="Trending topics">
          ...
          <div class="award-list">
            @foreach ($topTags->slice(4)->take(12) as $tag)
              <a class="award-badge" href="{{ route('categories.show.tag', ['category' => $newsCategory->slug, 'tagSlug' => $tag->slug]) }}">
                {{ $tag->name }}
              </a>
            @endforeach
          </div>
      ...
      @if (config('app.digest_testing'))
        <div class="content-grid" style="border: 1px solid red; padding: 20px; margin-top: 20px;">
          <section class="work-columns" aria-label="Top stories">
              <article class="category-column">
